...
Текущий overlay уже подтверждает расчет, запуск и live scan, но до production UX еще не дотягивает. Цель - экран в стиле легкого игрового overlay: минимум постоянных controls, понятный основной сценарий и служебные настройки вне первого экрана.

### P0: production UX cleanup

- [x] Убрать dev-тексты про скриншоты из пользовательского flow; скриншоты оставить только как режим разработки.
...
- [x] Подключить `Скан цепочки` к пошаговому режиму по нескольким NPC парам.
...
- [x] Добавить настройку размера шрифта и масштаба таблицы под 1080p/1440p/4K.
...
- [x] `Settings` dialog без дублирующей вкладки.
...
- [x] Калибровка OCR по умолчанию проверяет live область экрана/игры; скриншот оставлен только как dev fallback.
